...
export function PopupFilters({
  ...
  function apply(e: React.FormEvent) {
    ...
    const f = String(fd.get("frequency") ?? "");
    const a = String(fd.get("active") ?? "");
    if (s) sp.set("search", s);
    if (p) sp.set("placement", p);
    if (f) sp.set("frequency", f);
    if (a) sp.set("active", a);
    const qs = sp.toString();
    startTransition(() => router.push(`/admin/popups${qs ? `?${qs}` : ""}`));
  }

  function clear() {
    startTransition(() => router.push("/admin/popups"));
  }
  ...
  return (
    ...
    >
      ...
      <label className="block">
        ...
        <select name="frequency" defaultValue={frequency} className={fieldCls}>
